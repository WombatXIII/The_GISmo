...
def CreateInput(frame, details, rowPos):
    ttk.Label(frame, text=details.BeforeText)\
        .grid(column=2, row=rowPos, sticky=E)
    entry = ttk.Entry(frame, width=7, textvariable=details.EntryTextVar)
    entry.grid(column=3, row=rowPos, sticky=(W, E))
    ttk.Label(frame, text=details.AfterText)\
        .grid(column=4, row=rowPos, sticky=W)


def CreateOutputs(frame, details, rowPos):
    ttk.Label(frame, text=details.BeforeText)\
        .grid(column=2, row=rowPos, sticky=E)
    ttk.Label(frame, textvariable=details.EntryTextVar)\
        .grid(column=3, row=rowPos, sticky=(W, E))
    ttk.Label(frame, text=details.AfterText)\
        .grid(column=4, row=rowPos, sticky=W)
...
def CreateCalculator(parentFrame, details):
    ...
    # Creates the title
    ttk.Label(parentFrame, text=details.Title)\
    ...
    # Creates The Button
    ttk.Button(parentFrame, text="Calculate", command=details.Calculate)\
        .grid(column=4, row=rowPos, sticky=W)

    # This is hacky way of applying padding.
    # TODO We should remove this and apply padding to the componenets
    # individually so we don't accidentally override stuff.
